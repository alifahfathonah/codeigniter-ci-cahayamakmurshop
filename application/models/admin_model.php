...
class admin_model extends CI_Model {
	 ...
	 function getAllTransaksi() {
	 	$this->db->select('tbl_transaksi_header.*, (tbl_transaksi_detail.harga+tbl_transaksi_header.ongkos_kirim) as total');
    ...
    function getAllRetur() {
        ...
        $this->db->from('tbl_transaksi_header');
        $this->db->join('tbl_transaksi_detail', 'tbl_transaksi_header.kode_transaksi = tbl_transaksi_detail.kode_transaksi');
        $this->db->where('tbl_transaksi_header.status = "3"');
        $query = $this->db->get();
        return $query->result();
    }

    function GetDetailJumlahJual() {
    	$this->db->select('sum(tbl_transaksi_detail.harga+tbl_transaksi_header.ongkos_kirim) as jumlah');
        $this->db->from('tbl_transaksi_header');
        $this->db->join('tbl_transaksi_detail', 'tbl_transaksi_header.kode_transaksi = tbl_transaksi_detail.kode_transaksi');
        $this->db->where('tbl_transaksi_header.status = "1"');
        $query = $this->db->get();
		...
		function edit_data_resi($where,$table){		
  ...
  public function cek_user($username,$password){
	...
	function ResiUpdate($id_transaksi_header,$no_resi) {
		return $this->db->query("update tbl_transaksi_header set no_resi='$no_resi' where id_transaksi_header='$id_transaksi_header'");
	}
	...
	function SimpanBukuTamuAdd($email,$judul,$isi_buku_tamu_kirim) {
		return $this->db->query("insert into tbl_hubungi_kami_kirim values('','$email','$judul','$isi_buku_tamu_kirim')");	
	}
	...
	function UpdateTransaksiHeaderRetur($id) {
		return $this->db->query("update tbl_transaksi_header set status='3' where id_transaksi_header='$id'  ");
	}

	function UpdateTransaksiHeaderCancel($id) {
		return $this->db->query("update tbl_transaksi_header set status='4' where id_transaksi_header='$id'  ");
	}
	...
	function GetEmail($id){
		return $this->db->query("select email from tbl_transaksi_header where id_transaksi_header = $id");
	}
	...
	function GetTransaksiRetur() {

		return $this->db->query("select a.*,b.*,c.* from tbl_transaksi_header a
		join tbl_bank b on a.bank_id=b.id_bank
		join  tbl_jasapengiriman c on a.jasapengiriman_id=c.id_jasapengiriman
		where a.status='3' order by a.id_transaksi_header asc  ");

	}

	function EditTransaksi($id_transaksi_header){
		return $this->db->query("select a.*,b.*,c.* from tbl_transaksi_header a join tbl_bank b on a.bank_id = b.id_bank
			join tbl_jasapengiriman c on a.jasapengiriman_id = c.id_jasapengiriman
			where a.id_transaksi_header='$id_transaksi_header' ");
	}

	function transaksiUpdate($id_transaksi_header,$kode_transaksi,$penerima,$email,$alamat,$no_telepon,$propinsi,$kota,$kode_pos,$bank_id,$jasapengiriman_id,$nama,$nama_bank,$ongkos_kirim){
		return $this->db->query("update tbl_transaksi_header set kode_transaksi='$kode_transaksi',penerima='$penerima',email='$email',alamat='$alamat',no_telepon='$no_telepon',propinsi='$propinsi',kota='$kota',kode_pos='$kode_pos',bank_id='$bank_id',jasapengiriman_id='$jasapengiriman_id',ongkos_kirim=$ongkos_kirim
		 where id_transaksi_header='$id_transaksi_header'");
	}

	

	function update_ongkirtransaksi($where,$data,$table){
	  $this->db->where($where);
